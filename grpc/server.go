...
func NewServer(sdkClient sdk.Client, metrics metrics.Handler, conf config.Config, logger log.Logger, errorChan chan error) *Server {
...
func (s *Server) Listen() {
	...
	go func() {
		listener, err := net.Listen("tcp", fmt.Sprintf(":%d", s.conf.Grpc.Port))
		if err != nil {
