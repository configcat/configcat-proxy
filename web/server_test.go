...
func TestNewServer(t *testing.T) {
	...
	srv := NewServer(http.HandlerFunc(ServeHTTP), log.NewDebugLogger(), &config.Config{Http: config.HttpConfig{Port: 5050}}, errChan)
...
`, func(key string) {
			...
			srv := NewServer(http.HandlerFunc(ServeHTTP), log.NewDebugLogger(), &config.Config{Http: config.HttpConfig{Port: 5050}, Tls: tlsConf}, errChan)
